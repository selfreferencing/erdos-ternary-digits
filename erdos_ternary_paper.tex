...
\textbf{Case A:} $r \in \{1, 2\} \cup [4, 3^{12})$. The Lean formalization proves rejection for all such $j$ by extending the computational verification: for each $r \notin \{0, 3\}$ with rejection position $k_r$, the proof verifies that $j \equiv r \pmod{3^{k_r}}$ (which follows from $j \equiv r \pmod{3^{12}}$ when $k_r \leq 12$, and is handled separately for larger $k_r$). The key lemma \texttt{caseA\_transfer} establishes that the rejection at position $k_r$ transfers from $r$ to any $j \geq 3^{12}$ with $j \equiv r \pmod{3^{12}}$.
