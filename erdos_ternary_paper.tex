...
For $j \geq 1$, let $L(j)$ be the number of base-3 digits of $2 \cdot 4^j$. The value $j = 3$ is the unique $j \geq 1$ such that the automaton $\mathcal{A}$, after processing all $L(j)$ digits, has not rejected and is in a state that accepts the infinite tail of zeros.
\end{lemma}
...
For $j \geq 4$: $L(j) \geq 6$ since $2 \cdot 4^4 = 512 > 3^5 = 243$. The Lean formalization proves that every $j \in [4, 3^{12})$ is rejected before position 27. For $j \geq 3^{12}$, periodicity and 3-adic induction (Theorem~\ref{thm:unique}) establish rejection at bounded positions. Thus $j = 3$ is the unique survivor.
...
\textbf{Case A:} $r \in \{1, 2\} \cup [4, 3^{12})$. Since $r \notin \{0, 3\}$, the computational verification shows $r$ is rejected. The Lean formalization proves that rejection occurs before position 27 for all such $r$, and the first 27 digits of $2 \cdot 4^j$ match those of $2 \cdot 4^r$ (by periodicity mod $3^{26}$). Therefore $j$ is rejected at the same position as $r$.

\textbf{Case B:} $r = 3$, i.e., $j \equiv 3 \pmod{3^{12}}$ with $j \neq 3$. Write $j = 3 + m \cdot 3^{12}$ for $m \geq 1$. We prove rejection by induction on $\nu_3(m)$.

\textbf{Case C:} $r = 0$, i.e., $j \equiv 0 \pmod{3^{12}}$ with $j \neq 0$. Write $j = m \cdot 3^{12}$ for $m \geq 1$. The proof uses 3-adic induction on $m$:
\begin{itemize}
    \item If $m \equiv 2 \pmod 3$: digit 13 is 1, and $s_0$ rejects immediately.
    \item If $m \equiv 1 \pmod 3$: the Lean formalization proves rejection before position 27 via orbit coverage (the automaton enters $s_1$ at position 13 and must encounter a rejecting digit pattern within bounded positions).
...
\end{itemize}
The Lean formalization proves \texttt{caseC\_reject\_before27}: all rejections in Case C occur before position 27, making the induction well-founded.
