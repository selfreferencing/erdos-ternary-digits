...
\date{January 2025 \\ \small Version 8 (2025-01-25 19:45 UTC)}
...
\begin{abstract}
We prove that for all $n > 8$, the base-3 representation of $2^n$ contains at least one digit 2. The only exceptions are $n \in \{0, 2, 8\}$. Our proof uses a finite automaton that characterizes when doubling produces a 2, combined with the periodicity of powers of 4 modulo $3^K$ (via the Lifting the Exponent lemma). For $j \geq 3^{12}$, the proof proceeds by 3-adic induction: residue classes not congruent to 0 or 3 modulo $3^{12}$ are rejected by computational verification, while the remaining cases are handled via orbit coverage and digit shift arguments that reduce to smaller values. Among $j \geq 1$, the unique survivor is $j = 3$, because $2 \cdot 4^3 = 128 = (11202)_3$ has a digit sequence that avoids all rejection conditions (the case $j = 0$ is trivially accepted since $2 \cdot 4^0 = 2$).
...
\begin{itemize}
    \item $Q = \{s_0, s_1, s_R\}$ (carry 0, carry 1, and reject state)
    \item $\Sigma = \{0, 1, 2\}$ (base-3 digits)
    \item $q_0 = s_0$ (start with no carry)
    \item Transitions: $\delta(s_0, 0) = s_0$, $\delta(s_0, 2) = s_1$, $\delta(s_0, 1) = s_R$;\\
    $\delta(s_1, 0) = s_0$, $\delta(s_1, 1) = s_1$, $\delta(s_1, 2) = s_R$;\\
    $\delta(s_R, d) = s_R$ for all $d \in \Sigma$
\end{itemize}
The automaton accepts if it never enters state $s_R$. We say ``rejected at position $k$'' when the automaton first enters $s_R$ upon reading digit $k$.
\end{definition}

\begin{remark}[Digit Indexing Convention]
Digits are indexed starting at 0 from the LSB, so ``position $k$'' refers to the $(k+1)$-th digit from the right.
\end{remark}
...
\begin{lemma}[3-adic Expansion]\label{lem:lte}
...
Let $k \geq 1$. Let $T_k$ denote the number of survivors modulo $3^k$ after positions $0, 1, \ldots, k-1$. The survivors partition into orbits of size 3 under the map $j \mapsto j + 3^{k-1}$. Within each orbit, digit $k$ of $2 \cdot 4^j$ takes all three values $\{0, 1, 2\}$.
\end{lemma}
...
For each position $k \geq 1$, exactly $3 \cdot 2^{k-1}$ residue classes modulo $3^{k+1}$ have their first rejection at position $k$. The fraction is $2^{k-1}/3^k$, and $\sum_{k=1}^{\infty} 2^{k-1}/3^k = 1$.
\end{theorem}
...
\end{itemize}
So one residue class mod 3 rejects at position 1, and it has 3 lifts mod 9, giving $R_1 = 3$. The remaining $T_2 = 6$ survivors satisfy $T_2^{(0)} = 3$, $T_2^{(1)} = 3$.
...
\begin{align*}
T_{k+1}^{(0)} &= 3 \cdot \Bigl(\tfrac{T_k}{6} + \tfrac{T_k}{6}\Bigr) = 3 \cdot \tfrac{T_k}{3} = T_k \\
T_{k+1}^{(1)} &= 3 \cdot \Bigl(\tfrac{T_k}{6} + \tfrac{T_k}{6}\Bigr) = 3 \cdot \tfrac{T_k}{3} = T_k
\end{align*}
(The first term in each sum comes from survivors in $s_0$, the second from $s_1$.)
...
\]
This sum represents a measure-theoretic accounting: viewing residue classes as cylinder sets in $\mathbb{Z}_3$, the rejection sets have total measure 1. However, this density statement alone does not rule out a measure-zero exceptional set; the 3-adic induction in Theorem~\ref{thm:unique} provides the deterministic argument that among $j \geq 1$, only $j = 3$ survives.
\end{proof}
...
\section{The Unique Survivor ($j = 3$)}

\begin{lemma}[Finite Verification]\label{lem:finite}
For $1 \leq j < 3^{12}$, the automaton $\mathcal{A}$ accepts $2 \cdot 4^j$ if and only if $j = 3$. The maximum rejection position among rejected $j$ in this range is 36 (at $j = 124983$).
\end{lemma}

\begin{proof}
Verified computationally via \texttt{native\_decide} in Lean.

For the small cases: $j = 1$ gives $8 = (22)_3$ (LSB-first: $[2, 2]$), rejection at position 1. $j = 2$ gives $32 = (1012)_3$ (LSB-first: $[2, 1, 0, 1]$), rejection at position 3. $j = 3$ gives $128 = (11202)_3$ (LSB-first: $[2, 0, 2, 1, 1]$), no rejection---the sequence avoids all rejection conditions, and trailing zeros are always safe.
\end{proof}
...
\begin{itemize}
    \item $j = 1$: $2 \cdot 4 = 8 = (22)_3$ (LSB-first: $[2, 2]$). Trace: $s_0 \xrightarrow{2} s_1 \xrightarrow{2}$ \textsc{reject} at position 1.
    \item $j = 2$: $2 \cdot 16 = 32 = (1012)_3$ (LSB-first: $[2, 1, 0, 1]$). Trace: $s_0 \xrightarrow{2} s_1 \xrightarrow{1} s_1 \xrightarrow{0} s_0 \xrightarrow{1}$ \textsc{reject} at position 3.
\end{itemize}
...
\textbf{Case A:} $r \notin \{0, 3\}$ (i.e., $r \in \{1, 2, 4, 5, \ldots, 3^{12}-1\}$). Since $r \notin \{0, 3\}$, computational verification shows $r$ is rejected at some position $k_r$. For those $r$ with $k_r \leq 12$, rejection transfers to all $j \equiv r \pmod{3^{12}}$ by Lemma~\ref{lem:period} (since $j \equiv r \pmod{3^{12}}$ implies agreement on the first 13 digits, which suffices when rejection occurs by position 12). For $r$ with $k_r > 12$, the Lean formalization uses a 3-adic induction argument similar to Cases B and C: writing $j = r + m \cdot 3^{12}$, the proof analyzes the digit structure beyond position 12 and establishes rejection via orbit coverage and digit shift lemmas.
...
The first 5 digits match $128 = (11202)_3$ (LSB-first: $[2,0,2,1,1]$), ending in state $s_1$. Since $128 < 3^5 = 243$, digits 5--12 are zero. The automaton trace: $s_1 \xrightarrow{0} s_0 \xrightarrow{0} s_0 \cdots \xrightarrow{0} s_0$, reaching position 13 in state $s_0$.
...
\begin{itemize}
    \item $m \equiv 2 \pmod 3$: Digit 13 is 1 (since $2m \equiv 4 \equiv 1 \pmod 3$). State $s_0$ sees 1: \textsc{reject}.
    \item $m \equiv 1 \pmod 3$: Digit 13 is 2. State $s_0 \to s_1$. The Lean formalization proves rejection occurs within a bounded number of additional positions after entering $s_1$.
\end{itemize}
...
\begin{itemize}
    \item $m' \equiv 2 \pmod 3$: Digit is 1. State $s_0$ sees 1: \textsc{reject} at position $13 + t$.
    \item $m' \equiv 1 \pmod 3$: Digit is 2. State $s_0 \to s_1$. The digit shift lemma shows that the tail structure after position $13 + t$ is equivalent to the base case tail, so rejection occurs within a bounded number of additional positions.
\end{itemize}
...
The proof was discovered via iterative subdivision. At each position $k$, the rejection classes exhibit a regular pattern (Table~\ref{tab:coverage}).
...
This pattern guided the proof discovery. The actual proof uses 3-adic induction: for $m \equiv 1, 2 \pmod{3}$, orbit coverage forces rejection within a bounded number of positions after the first nonzero digit; for $m \equiv 0 \pmod{3}$, digit shift lemmas reduce to smaller 3-adic valuation. The Lean formalization verifies this structure completely.
